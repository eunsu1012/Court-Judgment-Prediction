...
지금까지의 모든 제출이 local OOF Accuracy(0.61~0.67)와 무관하게 **LB에서 0.50~0.51 사이**를 기록했다. 원인 분석 결과:

- **제출 파이프라인 버그 아님** — ID 정렬·중복 여부·컬럼 dtype 전부 정상 확인.
- 모델별로 "1"을 예측하는 비율이 72%~100%로 제각각인데도 LB는 전부 0.50 근처로 수렴 — 이는 **test set의 실제 클래스 비율이 train(66:34)과 달리 50:50에 가깝고**, 동시에 모델들이 여전히 실질적인 예측 신호를 학습하지 못했을 때에만 수학적으로 성립하는 패턴:

$$\text{Acc} = s \cdot q + (1-s)(1-q), \quad q \approx 0.5 \;\Rightarrow\; \text{Acc} \approx 0.5 \;\;(s\text{와 무관})$$

- 공개 판례 데이터베이스(Oyez/Justia)로 6건을 수동 대조한 결과 승소율 33%로 상반된 신호가 나왔으나, n=6은 통계적으로 무의미한 표본이라 1,240개 전체를 사용한 all-constant 제출 근거를 뒤집지 못함.
- **다음 실험 추천**: all-`0` 제출. LB가 여기서도 ≈0.5면 test 50:50 가설이 사실상 확정되고, ≈0.67이 나오면 "LB 고정 0.5" 현상은 다른 원인을 다시 찾아야 함.
